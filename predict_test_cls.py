...
test_data = d[0:100]
objs = map(lambda o: common.sample_one_input_data(o, common.NUM_POINT, rotate=True, rotate_value=[0,0, 88.5*np.pi/180]), test_data)
objs = [x for x in objs]
pcs = np.stack([x['points'] for x in objs], axis=0)
ang = np.array([common.angle_to_class(x['angle'])[2] for x in objs])
