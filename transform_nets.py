...
def input_transform_net(point_cloud, is_training, bn_decay=None, K=3):
    ...
    with tf.compat.v1.variable_scope('transform_XYZ') as sc:
        ...
        #                          initializer=tf.compat.v1.constant_initializer(0.0),
        #                          dtype=tf.float32)
        weights = tf.Variable(tf.zeros([256, 3*K]), dtype=tf.float32)
        biases = tf.Variable(tf.zeros([3*K]), dtype=tf.float32)
...
def feature_transform_net(inputs, is_training, bn_decay=None, K=64):
    ...
    with tf.compat.v1.variable_scope('transform_feat') as sc:
        ...
        #                          initializer=tf.compat.v1.constant_initializer(0.0),
        #                          dtype=tf.float32)
        weights = tf.Variable(tf.zeros([256, K*K]), dtype=tf.float32)
        biases  = tf.Variable(tf.zeros([K*K]), dtype=tf.float32)
